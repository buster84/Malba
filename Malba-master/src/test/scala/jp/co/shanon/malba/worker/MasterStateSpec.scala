...
class MasterStateSpec extends FlatSpec with Matchers with BeforeAndAfterAll {
  ...
  it should "set task type setting" in {
    ...
    state should be( MasterState( taskTypeSetting = HashMap( "taskType1" -> Tuple2( "aaa", Map.empty[String, String] ) ), tasks = HashMap.empty[String, CustomQueue]) )
    state = state.setTaskTypeSetting( "taskType2", "bbb" , 1 , Map.empty[String, String])
    state should be( MasterState( taskTypeSetting = HashMap( "taskType1" -> Tuple2( "aaa", Map.empty[String, String] ), "taskType2" -> Tuple2( "bbb", Map.empty[String, String] ) ), tasks = HashMap.empty[String, CustomQueue]) )
    state = state.setTaskTypeSetting( "taskType1", "aaa2" , 1 , Map.empty[String, String])
    state should be( MasterState( taskTypeSetting = HashMap( "taskType1" -> Tuple2( "aaa2", Map.empty[String, String] ), "taskType2" -> Tuple2( "bbb", Map.empty[String, String] ) ), tasks = HashMap.empty[String, CustomQueue]) )
  }
  ...
  it should "enqueue and dequeue task" in {
    ...
    state should be( MasterState( taskTypeSetting = HashMap.empty[String, ( String, Map[String, String] )], tasks = HashMap( "taskType1" ->  queue) ) )

    val ( task, newState ) = state.dequeue( "taskType1" )
    state = newState
    state should be( MasterState( taskTypeSetting = HashMap.empty[String, ( String, Map[String, String] )], tasks =  HashMap.empty[String, CustomQueue] ) )
  ...
  it should "delete a task by id" in {
    ...
    state should be( MasterState( taskTypeSetting = HashMap.empty[String, ( String,  Map[String, String])], tasks = HashMap( "taskType1" ->  queue1) ) )
    ...
    state should be( MasterState( taskTypeSetting = HashMap.empty[String, ( String, Map[String, String])], tasks = HashMap( "taskType1" ->  queue2) ) )
  ...
  it should "use custom queue of setting" in {
    ...
    queue1.enqueue( Task( "id1", "taskType1", "aaa1" ), None, Map.empty[String, String] )
    state should be( MasterState( taskTypeSetting = HashMap( "taskType1" -> Tuple2("jp.co.shanon.malba.queue.BalancingQueue", Map.empty[String, String]) ), tasks = HashMap( "taskType1" ->  queue1) ) )
  }
